"""
TypeScript/React/Next.js Bug Scanner — Phase 2 extension
Scans TypeScript/React/Next.js repositories using specialized tools.

Usage:
    python ts_agent.py <repo_path> [--output report.json]
    python ts_agent.py C:/Users/dicky/projects/next-store
"""
...
def create_mcp_client() -> MultiServerMCPClient:
    server_path = Path(__file__).parent.parent / "mcp-server" / "server_typescript.py"
...
async def scan_repository(repo_path: str) -> tuple[list[TSFinding], int]:
    mcp_client = create_mcp_client()
    tools = await mcp_client.get_tools()
    print(f"✅ Tools loaded: {[t.name for t in tools]}")

    # Get all TypeScript files
    print("\n📂 Listing TypeScript files...")
    files_result = await call_tool(tools, "list_ts_files", {
        "repo_path": repo_path,
        "file_type": "all",
        "include_tests": False,
    })

    ts_files = files_result.get("files", [])
    ...
    for i, file_info in enumerate(ts_files):
        ...
        client_tag = " [client]" if is_client else ""
        print(f"\n  [{i+1}/{len(ts_files)}] 🔍 {file_name} ({component_type}{client_tag})")
        ...
        for tool_name in analysis_tools:
            ...
            for f in findings:
                file_findings.append(TSFinding(
                    file=file_info["path"],
                    line=f.get("line", 0),
                    severity=f.get("severity", "low"),
                    category=f.get("category", "typescript"),
                    title=f.get("title", ""),
    ...
    return all_findings, len(ts_files)


# ─── AI Summary ───────────────────────────────────────────────────────────────

async def generate_ai_summary(findings: list[TSFinding], repo_path: str) -> str:
    if not findings:
    ...
    try:
        ...
        summary_text = f"Next.js/React repo: {repo_path}\nTotal: {len(findings)} findings\n\n"
        ...
        response = llm.invoke([
            SystemMessage(content=(
                "You are an expert React/Next.js/TypeScript engineer. "
                ...
                "Be specific about Next.js App Router, React hooks, and TypeScript "
                "best practices. Max 250 words."
            )),
...
def build_report(repo_path, findings, files_scanned, ai_summary) -> TSReport:
...
def print_report(report: TSReport) -> None:
    print("\n" + "═" * 60)
    print("  📘 TYPESCRIPT/REACT BUG REPORT")
    print("═" * 60)
    print(f"  Repo:    {report.repo_path}")
    print(f"  Scanned: {report.total_files_scanned} TypeScript files")
...
async def scan_repo(repo_path: str, output_file: str | None = None) -> TSReport:
    print(f"\n📘 TypeScript/React Scanner starting...")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python ts_agent.py <repo_path>")
        print("Example: python ts_agent.py C:/Users/dicky/projects/next-store")
        sys.exit(1)
