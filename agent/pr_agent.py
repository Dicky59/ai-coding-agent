"""
PR Review Agent — Phase 3 (v2)
Reviews GitHub Pull Requests by:
1. Fetching the PR diff from GitHub API
2. Running bug scanner on ALL changed Kotlin files (whole file)
3. Posting one inline comment per file with findings
...
async def get_pr_info(owner: str, repo: str, pr_number: int) -> dict:
    """Get PR metadata: title, branch, author, commit sha."""
...
def get_first_changed_position(patch: str) -> int | None:
    """
    Get the diff position of the first added line.
    Used to anchor file-level inline comments.
    """
    if not patch:
        return None
    position = 0
...
def format_file_comment(filename: str, findings: list[BugFinding]) -> str:
    """Format all findings for a file into one readable inline comment."""
    lines = [
        f"## 🤖 AI Review — `{Path(filename).name}`",
        f"Found **{len(findings)} issue(s)** in this file:",
        "",
    ]
...
async def review_pr(owner: str, repo: str, pr_number: int) -> None:
    ...
    if not kotlin_files:
        print("   No Kotlin files changed — nothing to review!")
        await post_comment(
    ...
    # Step 3: Download and scan files
    ...
    # Step 5: Build inline comments — one per file with findings
    inline_comments = []
    files_with_findings: dict[str, list[BugFinding]] = {}
    ...
    for filename, findings in files_with_findings.items():
        ...
        if position is None:
            print(f"    ⚠️  Could not get diff position for {Path(filename).name}")
            continue
    ...
    if not all_findings:
        ...
        action = "COMMENT"
        review_body = (
            f"Found {len(all_findings)} minor issue(s) (medium/low severity). "
            "Consider addressing them but not blocking merge."
        )
    else:
        action = "REQUEST_CHANGES"
        review_body = (
            f"Found {critical_or_high} critical/high issue(s) "
